...
const options = {
  key: readFileSync(path.join(__dirname, configJs.sslKey), 'utf-8'),
  cert: readFileSync(path.join(__dirname, configJs.sslCrt), 'utf-8')
}
...
httpsServer.listen(configJs.listenPort, () => {
  console.log('Listening on https://' + configJs.listenIp + ':' + configJs.listenPort)
})
...
async function createWorkers() {
  let { numWorkers } = configJs.mediasoup

  for (let i = 0; i < numWorkers; i++) {
    let worker = await createWorker({
      logLevel: configJs.mediasoup.worker.logLevel,
      logTags: configJs.mediasoup.worker.logTags,
      rtcMinPort: configJs.mediasoup.worker.rtcMinPort,
      rtcMaxPort: configJs.mediasoup.worker.rtcMaxPort
    })
